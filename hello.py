...
...
""" # NOTE: Full footer prompt text goes here.
...
async def process_voter_page_in_chunks_async(semaphore, page_image, prompt, page_index):
    ...
    midpoint = height // 2
    top_half, bottom_half = page_image.crop((0, 0, width, midpoint)), page_image.crop((0, midpoint, width, height))
    chunk_tasks = [
    ...
    for i, result in enumerate(chunk_results):
        if isinstance(result, Exception) or not result:
            ...
            continue
        combined_response_text += result.strip() + "\n"
        ...
        for i, line in enumerate(cleaned_response.split("\n")):
            if line.strip():
                try:
                    ...
                    logging.warning(f"Page {page_index+1}, Line {i+1}: Could not parse JSONL. Skipping.")
        return parsed_data
